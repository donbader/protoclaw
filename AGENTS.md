...
- **Lifecycle persistence**: `create_session()` calls `upsert_session(closed=false)` after success. `prompt_session()` spawns background `update_last_active()`. `shutdown_all()` leaves sessions open in the store — TTL-based expiry (`ttl_days`) handles cleanup. Sessions survive graceful restarts so `load_open_sessions()` can restore them on next boot. All store failures are logged (`tracing::warn`) but never block runtime operations.
- **Agent data persistence**: For session restore to work end-to-end, the agent's own data directory must survive container recreation. Configure `volumes` in `DockerWorkspaceConfig` to mount a named volume for the agent's data path (e.g. `XDG_DATA_HOME`). Without this, anyclaw restores the session mapping but the agent has no history to load from.
