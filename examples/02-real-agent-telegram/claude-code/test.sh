...
# Docker-only E2E test for Claude Code agent
...
MERGED_CONTENT=$(echo "$BATCH_OUTPUT" | grep -B5 'event: user_message_chunk' || true)
CONTENT_FOUND=0
for i in $(seq 1 5); do
  if printf '%s' "$MERGED_CONTENT" | grep -q "PROTO_BATCH_$i" 2>/dev/null; then
    CONTENT_FOUND=$((CONTENT_FOUND + 1))
  fi
done
if [ "$CONTENT_FOUND" -ge 3 ]; then
  pass "Merged prompt contains batch content ($CONTENT_FOUND/5 in SSE stream)"
else
  fail "Merged prompt missing batch content ($CONTENT_FOUND/5 in SSE user_message_chunk events)"
fi
