...
| `lib.rs` | `AnyclawConfig::load()` — Figment layered loading, `generate_schema()` |
| `schema_overlays.rs` | Post-process schemars output to inject extension-specific option schemas (e.g., Telegram `access_control`) |
...
- `WorkspaceConfig::Docker(DockerWorkspaceConfig)` — `image` (required), `entrypoint`, `volumes`, `env`, `memory_limit`, `cpu_limit`, `docker_host`, `network`, `pull_policy`, `working_dir` (optional, path inside the container sent as `cwd` in ACP `session/new`), `extra_hosts` (optional, `hostname:ip` entries added to container `/etc/hosts`)
...
## Schema Overlays (`schema_overlays.rs`)

`generate_schema()` produces the JSON Schema in two steps:
1. `schemars::schema_for!(AnyclawConfig)` — derives schema from Rust types
2. `apply_extension_overlays()` — injects extension-specific option definitions

Since `options` is `HashMap<String, Value>`, schemars renders it as a generic object. The overlay function adds typed `$defs` entries (e.g., `TelegramAccessControlConfig`) and wires them into `ChannelConfig.options.properties` so IDE autocomplete works for known extension options.

When adding a new extension option to the schema:
1. Add the `$defs` entry in `apply_extension_overlays()`
2. Wire it into the appropriate entity's `options.properties`
3. Run `cargo test -p anyclaw-config -- generate_and_write_schema --ignored` to regenerate
4. The drift guard test will fail CI if the committed schema doesn't match
