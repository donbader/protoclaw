    ...
    match name {
        ...
        "channels" => {
            ...
            let default_agent = config.default_agent_name()
                .unwrap_or("default")
                .to_string();
            let mut cm = ChannelsManager::new(config.channels_manager.channels.clone(), config.channels_manager.init_timeout_secs, default_agent, config.channels_manager.merge_window_ms)
