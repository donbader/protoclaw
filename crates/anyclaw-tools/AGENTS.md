...
- `anyclaw_tool_invocations_total` counter incremented after every tool call (`tool` + `status` labels; status is `"ok"` or `"error"`)
- `anyclaw_tool_duration_seconds` histogram recorded per invocation (`tool` label; value in seconds as f64)
- `tracing::info!(target: "anyclaw::audit")` emitted per invocation with `tool_name`, `success`, `duration_ms` structured fields; no separate log file — events flow through the tracing subscriber
