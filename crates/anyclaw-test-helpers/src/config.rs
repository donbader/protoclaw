...
pub fn sdk_channel_config() -> anyclaw_config::AnyclawConfig {
    let mut agents = HashMap::new();
    agents.insert(
        "default".to_string(),
        anyclaw_config::AgentConfig {
...
pub fn sdk_tool_config() -> anyclaw_config::AnyclawConfig {
    let mut agents = HashMap::new();
    agents.insert(
        "default".to_string(),
        anyclaw_config::AgentConfig {
    ...
    anyclaw_config::AnyclawConfig {
        ...
        tools_manager: anyclaw_config::ToolsManagerConfig {
            tools,
            ..Default::default()
        },
        supervisor: anyclaw_config::SupervisorConfig {
...
pub fn wasm_tool_config() -> anyclaw_config::AnyclawConfig {
    ...
    let wasm_path = std::env::temp_dir().join("anyclaw-test-echo.wasm");
...
pub fn multi_tool_config() -> anyclaw_config::AnyclawConfig {
    let mut agents = HashMap::new();
    agents.insert(
        "default".to_string(),
        anyclaw_config::AgentConfig {
...
pub fn invalid_tool_config() -> anyclaw_config::AnyclawConfig {
    let mut agents = HashMap::new();
    agents.insert(
        "default".to_string(),
        anyclaw_config::AgentConfig {
...
/// Config with a docker-workspace agent ("docker-agent") backed by `anyclaw-mock-agent:test`
/// and a debug-http channel. Uses `PullPolicy::Never` — image must be built first via
/// `build_mock_agent_docker_image()`.
pub fn docker_agent_config() -> anyclaw_config::AnyclawConfig {
...
pub fn docker_agent_config_with_options(
    ...
    agents.insert(
        ...
        anyclaw_config::AgentConfig {
            workspace: anyclaw_config::WorkspaceConfig::Docker(
                anyclaw_config::DockerWorkspaceConfig {
                    image: "anyclaw-mock-agent:test".to_string(),
                    entrypoint: None,
                    volumes: vec![],
                    env: HashMap::new(),
                    memory_limit: None,
...
pub fn build_mock_agent_docker_image() -> Result<(), String> {
    ...
    let output = std::process::Command::new("docker")
        .args(["build", "-t", "anyclaw-mock-agent:test", "."])
...
/// Stop and remove all containers managed by anyclaw that have the `anyclaw.managed=true` label.
///
...
mod tests {
    ...
    fn when_sdk_tool_config_called_then_echo_tool_is_mcp_type_with_correct_binary() {
        ...
        assert_eq!(tool.tool_type, anyclaw_config::ToolType::Mcp);
